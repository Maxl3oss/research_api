...
exports.getAllResearch = (req, res) => {
  try {
    ...
    db.query(query, (err, data) => {
      const qtyCount = query
        .replace(
          "research.id, research.title, research.date, research.isVerified, users.user_fname, users.user_lname",
          "COUNT(id)"
        )
        .replace("ORDER BY research.id DESC", "")
        .split(" ")
        .slice(0, -2)
        .join(" ");
...
exports.getFileById = (req, res) => {
  try {
    ...
    db.query(query, async (err, data) => {
      ...
      if (data.length) {
        return res.download(
          path.join(__dirname, "../uploads/pdf", data[0].file_pdf),
          (err) => {
            if (err) {
...
exports.getLimit = (req, res) => {
  try {
    ...
    if (search && type === "all") {
      query = query.replace(
        "HAVING research.isVerified=1",
        `WHERE (title LIKE "%${search}%" OR creator LIKE "%${search}%" OR description LIKE "%${search}%") AND research.isVerified=1`
      );
    } else {
      if (search && type) {
        query = query.replace(
          "HAVING research.isVerified=1",
          `WHERE research.isVerified=1 AND research.${type} LIKE "%${search}%"`
        );
    ...
    db.query(query, (err, data) => {
      const qtyCount = query
        .replace(
          "research.*, file_image AS image, CONCAT(users.user_fname ,' ', users.user_lname) AS user_name",
          "COUNT(id)"
        )
        .replace("HAVING research.isVerified=1", "WHERE research.isVerified=1")
        .replace("ORDER BY research.id DESC", "")
        .split(" ")
        .slice(0, -2)
        .join(" ");
...
exports.post = async (req, res) => {
  try {
    ...
    const query = `INSERT INTO  (file_pdf,file_image) VALUES ("${pdf_url}","${image_url}")`;
    db.query(query, (err, result) => {
      // insert files
      if (err) {
        return res.status(505).json({
          message: "errors insert files",
          data: err,
        });
      }
...
exports.getResearchByUser = (req, res) => {
  try {
    ...
    db.query(query, (err, data) => {
      ...
      const qtyCount = query
        .replace(
          "research.id, research.title, research.creator, research.date, research.isVerified, files.file_image AS image",
          "COUNT(id)"
        )
        .split(" ")
        .slice(0, -2)
...
exports.delResearchById = async (req, res) => {
  try {
    const { research_id } = req.body;
    const query = `SELECT research.file_id, files.file_pdf, files.file_image FROM research LEFT JOIN files ON (research.file_id=files.file_id) WHERE research.id = ${research_id}`;
    db.query(query, async (err, result) => {
      const { file_id, file_pdf, file_image } = result[0];
      db.query(
        `DELETE research, files, users_research FROM research INNER JOIN files ON research.file_id=files.file_id INNER JOIN users_research ON research.id=users_research.research_id WHERE research.id=${research_id}`
      );
      cloud.delete(file_pdf);
      cloud.delete(file_image);
